                    ...
                    if(isset($_POST['add'])){
                        ...
                        if(empty($name)||empty($email)||empty($age)||empty($addres)||empty($phone)||empty($dep)){
                            echo"<div class='alert alert-danger text-center'>الرجاء ملء الحقل ادناه </div>";
                                ...
                                $sql="INSERT INTO `students`(`name`, `email`, `age`, `addres`, `phone`, `depnum`) VALUES
                                ('$name','$email','$age','$addres','$phone','$dep')";
                                $result=mysqli_query($conn,$sql);
                                if($result){echo "<div class='alert alert-success text-center'>تم اضافة الطالب بنجاح  </div>";}
                                else{echo"<div class='alert alert-danger text-center'>حدث خطأ  </div>";}
                            
                        }
                    }
                    ?>
                    <div class="titbtn"><button class="btn btn-custom2">اضافة طالب جديد  </button></div>
